...
class Command(BaseCommand):
    ...
    help = 'Loads the records the given file type (ACSSF, PLST, uSF1, uSF3) and state abbreviation into the database'
    ...
    def handle(self, *args, **options):
        ...
        def acs(file_num, geo_type_dir):
            base_url = 'http://www2.census.gov/acs2005_2009_5yr/summaryfile/2005-2009_ACSSF_By_State_By_Sequence_Table_Subset/'
            path = os.path.join(
                state_name.replace(' ', ''), geo_type_dir,
                '20095%s%04d000.zip' % (state_abbr, file_num)
            )
            reader = remote_fallback_reader(
                '../data/acs2005_2009_5yr/summaryfile/2005-2009_ACSSF_By_State_By_Sequence_Table_Subset',
                base_url,
                path
            )
            ...
            z = ZipFile(reader)
            z.extractall('/tmp/')
            files = ['e20095%s%04d000.txt' % (state_abbr, file_num),
                'm20095%s%04d000.txt' % (state_abbr, file_num), ]
            for f in files:
                z.extract(f, '/tmp/')
